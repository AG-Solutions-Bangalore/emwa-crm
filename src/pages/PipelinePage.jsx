...
const initialForm = {
  ...
  subs: [
    {
      pipeline_sub_name: 'Step 1 - Welcome & Introduction',
...
export default function PipelinePage() {
  ...
  const fetchPipelineList = async (page = currentPage, query = searchQuery, status = statusFilter) => {
    setLoading(true);
    try {
      const params = {
        page,
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages && newPage !== currentPage) {
      ...
      fetchPipelineList(newPage, searchQuery, statusFilter);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    items.forEach((it) => {
      ...
      totalStages += count;
      const st = it.pipeline_status || it.status || 'Active';
      if (st === 'Active') active++;
      else inactive++;
    });

    return { total, totalStages, active, inactive };
  }, [items, totalCount]);
        ...
        <main className="flex-1 p-5 md:p-7 space-y-6 max-w-7xl w-full mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
            <div className="bg-white p-4 rounded-xl border border-[#E8E3DA] shadow-2xs flex items-center justify-between">
              <div>
                <p className="text-[11px] font-semibold tracking-wider text-[#8C8275] uppercase">Pipelines</p>
            ...
            <div className="bg-white p-4 rounded-xl border border-[#E8E3DA] shadow-2xs flex items-center justify-between">
              <div>
                <p className="text-[11px] font-semibold tracking-wider text-[#8C8275] uppercase">Total Steps</p>
                <h3 className="text-xl font-bold text-[#1A1817] mt-0.5">{stats.totalStages}</h3>
              </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-[#E8E3DA] shadow-2xs flex items-center justify-between">
              <div>
                <p className="text-[11px] font-semibold tracking-wider text-[#8C8275] uppercase">Active</p>
